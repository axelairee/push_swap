...
/*   Updated: 2024/01/29 16:21:38 by abolea           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_findformat(const char *format, va_list args)
{
	...
	i = 0;
	if (*format == 'c')
		i = ft_putchar_fd_p(va_arg(args, int), 1);
	if (*format == 'i' || *format == 'd')
		i = ft_putnbr_fd_p(va_arg(args, int), 1, &i);
	if (*format == 's')
		i = ft_putstr_fd_p(va_arg(args, char *), 1);
	if (*format == 'u')
